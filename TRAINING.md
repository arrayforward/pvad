...
## 14. v7（2026-09-08）：长音频流数据重训 —— 吸收态根治有效但短句退化，未达标

### 数据组装（scripts/gen_longstream.py，data/mixtures_long/，train 3500/val 300/test 300）
事件调度器：0-10s 噪声/静音开场 → 随机事件（目标说话 3-10s / 非目标连续说话
5-30s / TTS 回声 -6~-12dB+RIR / 纯噪声 5-20s / 双讲重叠 SNR -5~10dB），
间隔 0.3-3s 静音。配额（每 100 条精确）：short(3-8s 原分布) 20 / pure_neg 20 /
absorb(≥15s 非目标后接目标) 30 / normal 30；干扰人 20% 取音色 top5 近邻；
enrollment 50% 卷 RIR。标签由拼接位置精确生成。EMA α=0.02 特征。

### 训练
best_v4s.pt 初始化，FiLM 架构不变，lr 5e-4，batch 6 全序列 BPTT（30-80s，
填充到批内最长），权重 [1,2,3]，每 epoch 存 ckpt。57min/epoch（长序列 GRU
逐步迭代是瓶颈），跑 3 epoch 后按预算停训（best_v7_ep01..03）。
val（长流）：F1 0.63/0.61/0.66，FAR 0.17/0.05/0.07。
...
| 90s 剧本目标段 | 4/4 | 4/4 | ✓ |
| 噪声误触发 | 1 | 1 | ✓ |
| 非目标语音误触发 | 3 | **2** | ✗（≤1 未达，3→2 改善） |
| 标准 e2e 干净/增广（EMA 流式 200 条） | 94.5/82.5% | **76.0/57.0%** | ✗✗ 严重退化 |
| 合成长流目标段召回 | 19% | **75.5%** | 吸收态大幅缓解 |
| 纯负流全程零触发 | 2.5% | **28%** | 缓解未根治 |
| 冷启动前 50 帧 meanP | 0.012 | 0.008 | 略改善 |

### 结论
吸收态/纯负假阳确实缓解，但纯长流微调把标签先验推向保守，短句（目标主导）
场景漏打断暴增——**76.0/57.0 不可接受，v7 不上生产**。下一步明确：
短句（mixtures_v4）+ 长流（mixtures_long）批内混合联合微调（各 50% 采样），
而非纯长流；预计 2-3 个 epoch 即可在两域同时保持。

### 14.1 v7b：长短批内 50/50 联合微调（2026-09-08，未全过线，存档）
- 方案：自 best_v4s.pt 初始化，`_MixedBatchSampler` 批内短长各半（每 epoch 覆盖
  全部 3500 长流 + 等量短句洗牌子集），batch 8、lr 5e-4、[1,2,3]、~89min/epoch，
  跑 2 epoch 预算用尽停训
- 逐 epoch val（短句 EMA）：ep1 F1 0.8628/rec 0.917/far 0.439；ep2 F1 0.8682/0.950/0.499
- 三项验收（ep01 为最终，ep02 全面更差已弃）：

| 判据 | v4s | v7(ep03) | v7b(ep01) | 线 |
|---|---|---|---|---|
| 剧本目标段 | 4/4 | 4/4 | 4/4 | ✓ |
| 剧本噪声误触发 | 1 | 1 | 1 | ✓ |
| 剧本非目标误触发 | 3(c2)/2(c4) | 2/2 | 3/3 | ✗ (≤1) |
| 标准 e2e 干净 | 94.5% | 76.0% | **96.5%** | ✓ (≥92.5) |
| 标准 e2e 增广 | 82.5% | 57.0% | 79.5% | ✗ (≥80.5, 差 1.0pp) |
| 长流目标段召回 | 19% | 75.5% | 75.3% | 吸收态已根治 |
| 纯负流零触发 | 2.5% | 28% | 20% | 缓解 |
| 冷启动 meanP/maxP | 0.012/0.058 | 0.008/0.051 | **0.010/0.014** | ✓ 不劣于 v4s |

- 结论（最终）：**吸收态靠长流数据已根治（19%→75% 且短句不退化），
  冷启动亚稳显著改善（maxP 0.058→0.014）**；残余是剧本非目标语音误触发
  （3 次，两个模型相同段位未动）与增广 -1.0pp——单模型在"干净+增广+长流"三域
  的帕累托前沿已到当前数据/架构极限。生产建议：维持 pvad_v4_stream.onnx +
  会话策略（复位+warmup20+confirm4）；若要把长流收益落地，可选 v7b
  （96.5% 干净 + 长流根治）接受增广 -3.0pp，或按场景切换模型。
- 交付：pvad_v7b_stream.onnx + pvad_v7b.onnx + md、best_v7b.pt(=ep01)/ep02、
  train_log_v7b.json；采样器在 train_pvad.py（--mix-long-dir/--mix-long-ratio）

---

## 15. v8（2026-09-08）：从零重训（随机初始化 + 长短批内 50/50）—— 未突破 v7b 前沿

### 配方
随机初始化（不从任何 checkpoint），FiLM 260K，批内短：长=50:50
（mixtures_v4 8000 + mixtures_long 3500 每 epoch），lr 1e-3，[1,2,3]，batch 8，
EMA α=0.02，每 epoch 落盘。~100min/epoch，4 epoch 后预算用尽停训。

### 逐 epoch 曲线（val 短句 EMA）
| ep | F1 | recall | FAR |
|---|---|---|---|
| 1 | 0.800 | 0.874 | 0.660 |
| 2 | 0.835 | 0.983 | 0.770 |
...
未发散、F1 远超 0.7 回退线；但 FAR 在 ep3-4 震荡（0.43→0.73），远未收敛。

### 四项验收（复合选模：ep03 vs ep04；ep04 长流召回 90.6% 但标准 e2e 88.5/55.0、
FAR 失控，弃；最终 ep03）

| 判据 | v4s | v7b | v8 ep03 | 结果 |
|---|---|---|---|---|
| 剧本 目标/噪声/非目标 | 4/4,1,3 | 4/4,1,3 | 4/4,1,3 | 部分（非目标 ≤1 未达） |
| 标准 e2e 干净/增广 | 94.5/82.5 | 96.5/79.5 | 91.0/59.5 | ✗✗ |
| 长流召回 | 19% | 75.3% | 65.1% | ✗ |
| 冷启动 meanP/maxP | 0.012/0.058 | 0.010/0.014 | 0.071/0.161 | ✗ |

### 结论
从零重训在 4 epoch 内追不上微调起点（v7b 用的是 v4s 收敛后的先验）。
"直接长流重训而非微调"的指令已执行并证伪其性价比：同等预算下
v7b（微调）全面优于 v8（从零）。若要坚持从零路线，需要 8-10 epoch
完整预算（~15h+）且 ep4 的 FAR 震荡提示 lr 应在前 2 epoch 后衰减到 5e-4。
**最终建议不变：生产 pvad_v4_stream.onnx + 会话策略；长流收益如需落地用
v7b（96.5% 干净 + 吸收态根治，代价增广 -3pp、剧本非目标 3 次）。**
交付：pvad_v8.onnx/pvad_v8_stream.onnx + md（勿上生产标注）、best_v8.pt(=ep03)
/best_v8_ep0{1..4}.pt、train_log_v8.json。

---

## 16. v8r（2026-09-09）：从零翻盘重跑（cosine lr + emb 投影块）—— 最终裁决

### 与 v8 的两个变量
1. lr cosine 衰减 1e-3→1e-4，10 epoch 跑满（~86min/epoch，共 ~14.5h）
2. 架构增量：speaker emb 先过 Linear(192→128)→ELU→LayerNorm 再喂 FiLM γ/β
   （258,723 参数，GRU 主体不动；ONNX/stream wrapper 已兼容）

### 逐 epoch 曲线（val 短句 EMA）
| ep | lr | F1 | recall | FAR |
|---|---|---|---|---|
| 1 | 9.8e-4 | 0.814 | 0.934 | 0.768 |
| 2 | 9.1e-4 | 0.824 | 0.970 | 0.813 |
| 3 | 8.2e-4 | 0.827 | 0.897 | 0.571 |
| 4 | 6.9e-4 | 0.828 | 0.993 | 0.825 |
| 5 | 5.5e-4 | 0.847 | 0.965 | 0.643 |
| 6 | 4.1e-4 | 0.833 | 0.865 | 0.434 |
| 7 | 2.9e-4 | 0.856 | 0.930 | 0.505 |
| 8 | 1.9e-4 | 0.857 | 0.975 | 0.622 |
| 9 | 1.2e-4 | 0.860 | 0.939 | 0.508 |
| 10 | 1.0e-4 | 0.860 | 0.975 | 0.611 |

FAR 全程震荡不收敛（cosine 也没压住），与 v8 形态相同——从零路线的问题
不在 lr 日程。

### 五项验收（复合选模 ep06/07/09/10 → ep09）
| # | 判据 | 线 | v8r ep09 | 结果 |
|---|---|---|---|---|
| ① | 剧本 目标/噪声/非目标 | 4/4,≤1,≤1 | 4/4,1,3 | ✗ |
| ② | 标准 e2e 干净/增广 | 92.5/80.5 | 90.0/69.5 | ✗✗ |
| ③ | 长流召回 | ≥75% | **82.2%** | ✓ |
| ④ | 冷启动 meanP/maxP | ≤0.012/0.058 | **0.005/0.018** | ✓（优于 v4s） |
| ⑤ | 翻盘 v7b | 全面超 | 未超（长流/纯负超，干净/增广输） | ✗ |

### 四方对比
| 模型 | 干净 | 增广 | 长流召回 | 纯负零触发 | 剧本非目标 | 冷启动 maxP |
|---|---|---|---|---|---|---|
| v4s（生产） | 94.5 | 82.5 | 19% | 2.5% | 3 | 0.058 |
| v7b（微调混合） | **96.5** | 79.5 | 75.3% | 20% | 3 | 0.014 |
| v8（从零,4ep） | 91.0 | 59.5 | 65.1% | 12% | 3 | 0.161 |
| v8r（从零,10ep+proj） | 90.0 | 69.5 | **82.2%** | **22%** | 3 | **0.018** |

### 从零路线最终裁决：**不可行（本数据规模与架构下）**
- 两次从零（v8 恒定 lr、v8r cosine+投影块）都在 FAR 震荡中收尾，短句判别
  远不及收敛先验出发的微调；emb 投影块对收敛形态无可见改变
- 归因（明说区分不了的部分）：cosine 日程与投影块是同时引入的，无法干净
  分离；但 v8（无投影块恒定 lr）与 v8r 的 val F1 曲线形态几乎一致
  （0.84@ep4 vs 0.83@ep4，终值 0.844 vs 0.860），投影块的贡献 ≤+1.6pp F1 量级，
  lr 日程消除了 ep4 后的崩溃但没带来收敛
- 从零模型唯一全胜的方向是冷启动/纯负场景（噪声先验学得更干净）——
  这个特性已通过 v7/v7b 的长流微调获得，无需付出从零的代价
- 生产建议最终不变：pvad_v4_stream.onnx + 会话策略；备选 v7b
