...
- **四种模型版本**：pvad.onnx (v1) / pvad_v2.onnx (增广) / pvad_v3.onnx (FiLM) /
  pvad_v4.onnx (FiLM，**当前默认**——易混淆硬负例训练，两口径双优)，接口相同可直接替换
...
| 测试集 | **PVAD v4（默认）** | PVAD v3 | PVAD v2 | AS-norm 基线 |
|---|---|---|---|---|
| v1 干净 | 1.0% / 5.0% / **94.0%** | 0.6% / 7.0% / 92.4% | 0.4% / 14.6% / 85.0% | 64.8% / 0% / **35.2%** |
| v2 增广（噪声+混响） | 0.6% / 16.4% / **83.0%** | 0.2% / 21.6% / 78.2% | 0.2% / 18.0% / 81.8% | 74.0% / 0% / 26.0% |

C++ 生产管线口径（`double_voice --batch-list`，v4 + RNNoise 默认开，200 条/条件，
...
v4 帧级（干净/增广）：recall 0.940/0.926，FAR 0.210/0.446（历代最优）。
