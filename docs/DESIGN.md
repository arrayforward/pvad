...
**后续两条探索线（一句话结论，详见 TRAINING.md 第 12/13 节）**：

- **v5s（cos 注意力 + EMA 微调流式）**：注意力打分做 cosine 归一化，消除漂移敏感
  （未微调即 87.0% vs 未归一化 71.0%）；EMA 微调后流式 **92.0% / 85.0%**
  （干净过线、增广差 2pp 未达 v5 离线的 89.4%）——可作为实时干净场景的可选模型。
- **v6（per-frame CMVN 重训）**：证明"训练分布=部署分布"可行（流式≡离线 0.0pp 逐数字
  相同），但逐帧减均值抹掉段级能量动态，增广判别信息损失（增广 77.5%，帧 FAR
  0.17-0.24 vs v5fa 0.10）——**跨帧统计本身承载判别线索，EMA-CMVN 微调这种
  "近似但保留统计"的方案是最优折中**，v6 方向终止。
...
1. **enrollment 多帧表示 + 真注意力**（v5 tokens 已落地离线；剩余是把多帧表示
   推广到流式路径且增广达标——v5s 已证明 cos 归一化有效但增广仍差 2pp）；
...
**已探索并关闭（有结论）**：注意力输入归一化（cos-attn，v5s：消除漂移敏感，
87.0% vs 71.0% 未微调）；per-frame CMVN（v6：训练分布=部署分布可行，
流式≡离线 0.0pp，但逐帧减均值抹掉段级能量动态、增广 77.5%——
**跨帧统计本身承载判别线索，EMA-CMVN 微调是最优折中**，方向终止）。
