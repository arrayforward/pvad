...
RNNoise 在本系统上的收益非常有限：①干净条件下确实让 A 更稳（0.68→0.80）、B 更低
（0.48→0.38），但干净条件本来就不需要它；②对加白噪的 SAPI 语音反而有失真风险
（A 在 SNR10 漏触发、echo/B 的 P 被抬升）；③对真实双讲混合物（含噪声混响）
判定零改善——**系统瓶颈是 PVAD 在域外/强干扰下的 FAR，不是可加性底噪**。
建议：默认关；仅"安静会议室但有持续平稳底噪（风扇/空调）"场景手动开启。
