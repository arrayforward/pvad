...
confirm4）不变**。原因：v7b 虽根治吸收态且干净反超，但增广 −3.0pp（79.5% vs 82.5%
...
**长流系列状态：已关闭**。生产决策最终版：pvad_v4_stream.onnx + 会话策略
（VAD 复位 + warmup20 + confirm4）；可选存档 pvad_v7b_stream.onnx
（干净 96.5% + 吸收态根治，代价增广 −3pp）。
