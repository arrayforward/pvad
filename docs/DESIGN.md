...
### 4.4 长流持续监听的"吸收态死亡"（生产 bug 复盘，有数值记录）

**症状**：实时持续监听开始时识别率高，约 1 分钟后急剧下降（CLI --mic 与 qt_demo
麦克风路径，PvadStream + pvad_v4_stream）。

**复现**（90s 剧本流，test_audio/longstream/，scripts/longstream_check.py：
噪声10s→目标5s→旁人10s→回声15s→目标5s→噪声15s→目标5s→旁人10s→噪声15s→目标5s）：

- 原始整流：前 25s 一切高 P（冷启动，噪声 mean 0.82/目标 0.71/旁人 0.50），
  **回声段（15s 连续旁人语音）后模型彻底死亡**——后续 3 个目标段 P<0.02 全部漏检，
  直至 95s 流结束。整段 v4（full CMVN，无 EMA）在同音频上同样死亡。
- **根因（双边验证）**：不是隐状态无界（||h|| 全程 10-14 有界），也不是 EMA 漂移
  （full CMVN 同现）——是单向 GRU 的隐状态在长时间连续非目标语音后进入
  **"一切皆非目标"的吸收区**：模型只见过 3-8s 训练片段，60s+ 连续流是分布外状态，
  属模型行为（python onnxruntime 与 C++ 数值一致），非实现 bug。
- **冷启动的另一面**：GRU 零状态起始是亚稳的"一切像目标"区（本剧本中持续 ~20s，
  MUSAN 大噪声上甚至整段持续高 P）。所以"每段新会话"式频繁复位不可行（§4.3
  的二次冷启动教训在此被放大）。

**修复策略**（python 多策略网格验证后定型，CLI StreamRunner 与 qt_demo 麦克风同策略）：

1. **VAD 门控打分**：只在 silero VAD 判语音（p>0.5）的帧上更新门控——MUSAN 噪声段
   VAD 恒低（frac 0.00-0.01），天然免假阳；
2. **VAD speech-end 会话分段复位**（`reset_gru`：只清 GRU state，**保留 EMA 与
   enrollment 先验**避免 CMVN 冷启动）——解除吸收态，后续目标段恢复触发；
3. **复位后 warm-up 20 个 VAD 帧 + confirm=4** 压冷启动尖峰。

**修后 90s 剧本结果**（`double_voice --long-stream-test`）：目标段 **4/4 触发**
（修复前 1/4），噪声段 1 次触发（VAD blip 角落），非目标语音段 3 次触发
（模型冷启动 FAR 的残余——多项 warm-up/confirm 网格证明无法在不伤目标段的前提下
进一步压低；长流重训已做完并关闭（v7/v7b/v8，见下）。回归判据：目标段全触发 +
噪声段 ≤1 + 非目标语音段 ≤3（python 双边验证的上限）。整段路径不受影响。

**长流重训复盘（v7/v7b，2026-08-28，结论：生产不变）**：

组装长流数据（`scripts/gen_longstream.py`，data/mixtures_long/）重训了两轮：

| 配方 | 吸收态召回 | 干净 e2e | 增广 e2e | 90s 剧本非目标误触发 | 结论 |
|---|---|---|---|---|---|
| v7（纯长流微调） | 19%→**75.5%** | **76.0%（崩溃）** | — | — | 纯长流是错误配方：长流治好了，短句先验被破坏 |
| **v7b**（批内 50/50 长短混合，best_v7b.pt=ep01） | 19%→**75.3%** | **96.5%**（超 v4s 的 94.5%） | 79.5%（差线 1.0pp） | 3 次（与 v4s 同） | 三域帕累托前沿到顶 |

**最终决策：生产维持 pvad_v4_stream.onnx + 会话策略（VAD 复位 + warmup20 +
confirm4）不变**。（**2026-09-08 更新：用户复核后采纳 v7b 为实时默认，见本节末尾
"v7b 采纳"**。原文保留作决策史。）原因：v7b 虽根治吸收态且干净反超，但增广 −3.0pp
（79.5% vs 82.5%
差线 1.0pp）且 90s 剧本非目标误触发未改善（3 次同 v4s）——会话策略已把 v4s 的
长流行为拉到可用，换模型的收益不抵增广退步。`pvad_v7b_stream.onnx` 作为可选模型
存档（部署建议见其 md）；v7 标注勿用。

**方法论沉淀**：单模型三域（干净/增广/长流）通吃已到极限；纯长流微调会破坏短句
先验——**批内长短混合才是正解**；残余误触发与吸收态的根子都是冷启动/FAR，下一步
收益最大的方向是评估驱动选模 + 硬负例继续加码，而非再改归一化方案。

**v8：从零重训证伪（2026-09-08，长流系列至此全部关闭）**：

最后一轮：抛弃微调，用 v7b 的 50/50 长短混合数据**从零重训**（4 epoch × ~100min
预算，models/pvad/pvad_v8*.onnx）。四项验收均未过线：干净 91.0%（线 94%）、
增广 59.5%（线 78%）、长流召回 65.1%（线 75%，即 v7b 水平）、冷启动更差。
**从零路线在该预算下被证伪**——v7b（自 v4s 微调）同预算全面占优：已训练好的
GRU 表征 + 温和微调远比从零重学划算。

复盘建议（喂给下一轮）：该预算下从零重训的失败大概率是 **lr 衰减不足**——
4 epoch 内学习率线性衰减到 0 的日程对"长流新任务 + 短句保持"双目标过紧；
若再试，应先验证衰减日程（cosine 到 0.1× 而非 0，或预算加倍），再谈数据配方。
但注意：即便 v8 过线，也需要同时赢过 v7b 的三域前沿才有生产意义。

**长流系列状态：已关闭**。**生产决策最终版（2026-09-08 更新，v7b 采纳）**：
实时 = pvad_v7b_stream.onnx + 会话策略（VAD 复位 + warmup20 + **confirm=2**；
v7b 自身免疫吸收态，confirm 从 4 降回 2 更灵敏——90s 剧本 confirm=4 会漏掉首个目标段、
confirm=2 全中且非目标误触发不恶化仍 3 次）；离线 = pvad_v5.onnx；
备选 = pvad_v4_stream.onnx（持续强噪/混响环境，增广 82.5% 最优但需吸收态策略）；
存档 = pvad_v5s_stream.onnx（干净可选）；v5_stream/v6/v8/v8r 禁用或终止。
长流重训（v7/v7b/v8/v8r）与推理侧融合（双窗口三轮）探索全部收敛关闭。

**v8r：从零路线的最终裁决（2026-09-08，系列正式关闭）**：

v8 证伪后给从零路线最后一次公平机会：从零 + **cosine lr 衰减** + **emb 投影块
（film_ebp）**，10 epoch 跑满 ~14.5h。四方对比（v4s / v7b / v8 / v8r）：

| 模型 | 干净 | 增广 | 长流召回 | 冷启动 maxP |
|---|---|---|---|---|
| v4s（生产实时） | 94.5% | 82.5% | 19% | 0.058 |
| v7b（存档可选） | 96.5% | 79.5% | 75.3% | — |
| v8（从零，线性衰减） | 91.0% | 59.5% | 65.1% | 更差 |
| v8r（从零 + cosine + 投影块） | 90.0% | 69.5% | **82.2%（四方最优）** | **0.018（四方最优）** |

**裁决：从零路线不可行**。v8r 把长流召回和冷启动压到四方最优，证明容量/初始化
不是瓶颈；但干净/增广全面输 v7b——此数据规模 + FiLM 架构下从零训练 FAR 全程震荡
不收敛，emb 投影块贡献 ≤+1.6pp F1，不构成翻盘。无法干净分离的部分明说：投影块与
cosine lr 的收益无法完全归因到各自（未做单变量消融），但二者同向且量级都不足以
改变裁决。

**"增大参数有效吗"的系列证据链**：v8r 从零训练的模型长流特性甚至优于微调系列，
说明容量与初始化均非瓶颈——瓶颈在**训练数据的分布覆盖与微调先验**（v4s/v7b 的
优势全部来自"已训练表征 + 温和微调"这条路径）。

**双窗口交替（staggered dual-window）实验（2026-09-08，结论：否决，部分过线）**：

思路：两个流实例错开半窗长跑（复位点错开 W/2），单实例流长永不超过 W（进不了
吸收态），融合永远取已过冷启动的成熟实例。`scripts/dual_window_eval.py`，
v4s 权重，网格 W∈{6,8,12}s × 成熟度阈值∈{10,20,30 帧} × 融合规则 {older, max}
（val 网格选型，test 只跑一次）。

网格结果（val n=60：目标段召回/非目标误触发率/纯负零触发率；90s 剧本：目标/噪声/
非目标）：

| 配置 | val 召回 | val 误触发 | val 纯负净 | 90s 剧本 |
|---|---|---|---|---|
| 基线（单实例+会话策略） | 0.293 | 0.195 | 0.412 | 4/4, 2, 2 |
| W=6 max warm=10 | **0.595** | 0.412 | 0.176 | 4/4, 1, 2 |
| W=8 max warm=10 | 0.543 | 0.384 | 0.235 | 4/4, 1, 2 |
| W=8 older warm=20 | 0.284 | **0.108** | **0.588** | **4/4, 1, 1** |
| W=12 max warm=10 | 0.474 | 0.291 | 0.294 | 4/4, 0, 1 |
| W=12 older（任意 warm） | 0.207 | 0.065 | 0.765 | 2/4, 0, 1 |

test 报数（n=300，与基线同条件）+ 短句退化（data/mixtures/test 前 100 条）：

| 配置 | test 目标召回 | test 误触发 | test 纯负净 | 短句 ok |
|---|---|---|---|---|
| 基线 | 0.251 | 0.194 | 0.370 | 0.940 |
| W=6 max warm=10 | **0.541** | 0.429 | 0.123 | **0.970** |
| W=8 older warm=20 | 0.237 | **0.105** | **0.593** | 0.910 |

**采纳判定（三线标准：长流召回≥75%、剧本非目标 ≤1、短句退化 ≤1pp）**：

- 长流召回：最优 54.1%（max 融合把召回从 25.1% 翻倍，证明"复位封顶吸收态损伤"
  的机制成立），但距 v7b 重训的 75.3% 差距大 ✗
- 剧本非目标误触发：W=8 older 压到 1（基线 2，v4s+会话策略 C++ 版为 2-3）✓
- 短句退化：max 融合反而 +3.0pp（0.940→0.970），older 融合 −3.0pp ✗（依配置）

**部分过线，不做 C++ 化**。trade-off 结论：双窗口是召回/误触发的一个新旋钮
（max 规则买召回、older 规则买纯净），但两个方向都无法同时超过"v4s+会话策略"
与"v7b 重训"的既有前沿；且机制上它把召回上限钉在"窗口中段的成熟区"——
冷启动区永远占流长的 warm/W 比例，W 越小召回越高但冷启动占比越大误触发越高。
残余收益空间在长流重训路线（v7b 配方），不在推理侧。

**第三轮：mean / maturity 加权平均融合（用户提议，2026-09-08，结论：连续谱证实，仍否决）**：

新规则：mean = (PA+PB)/2 过 θ 门控（θ∈{0.4,0.5,0.6,0.7}）；wmean = 成熟度加权平均
（w=min(1,窗龄/τ)，τ∈{10,20}帧）过 θ 门控（θ∈{0.4,0.5,0.6}）；W ∈ {6,8}s，
val n=60 网格选型（20 格，每格含 90s 剧本 + 短句 100 条），test 只跑一次。

网格精选（val 召回/误触发/纯负净 | 90s 目标/噪声/非目标 | 短句 ok）：

| 配置 | val 召回 | val 误触发 | 纯负净 | 90s | 短句 |
|---|---|---|---|---|---|
| mean W=6 θ=0.4 | 0.483 | 0.269 | 0.353 | 2/1/1 | 0.970 |
| mean W=8 θ=0.4 | 0.405 | 0.223 | 0.412 | **4/0/1** | 0.950 |
| mean W=8 θ=0.5 | 0.328 | 0.115 | 0.471 | 3/0/1 | 0.920 |
| mean W=8 θ=0.6 | 0.276 | 0.080 | 0.588 | 2/0/1 | 0.880 |
| mean W=8 θ=0.7 | 0.198 | 0.062 | 0.706 | 2/0/1 | 0.840 |
| wmean（任意 τ，≈mean） | 同上 ±0.01 | 同上 | 同上 | 同上 | 同上 |
| （对照）older W=8 warm=20 | 0.284 | 0.108 | 0.588 | 4/1/1 | 0.910 |
| （对照）max W=8 warm=10 | 0.543 | 0.384 | 0.235 | 4/1/2 | 0.970 |

发现：①wmean 与 mean 几乎逐行相同（τ≪W 时窗龄绝大多数时间已满权，平滑版无额外收益）；
②**连续谱假设证实**：mean 系随 θ 单调落在 older↔max 之间；③最优折中点
（mean W=8 θ=0.4：召回 0.405/误触发 0.217）轻微优于 older-max 帕累托连线在该召回处的
插值（连线误触发 ≈0.237，实测 0.217，优 0.02），且 90s 剧本噪声段首次归零——
但幅度不构成翻盘。

test 报数（W=8 mean θ=0.4，n=300）：召回 0.379 / 误触发 0.220 / 纯负净 0.321 / 噪声 0.006。
三线判定：长流召回 ≥75% 未达（最优仍 54.1%，max 规则）✗ → 采纳否决，不做 C++ 化。
双窗口三轮探索（older/max/mean/wmean × W/成熟度网格）至此收敛完毕：推理侧融合的
帕累托前沿已画清，残余空间确认在长流重训路线（v7b 配方）。
...
**长流重训**（v7 纯长流短句崩溃；v7b 批内混合达三域帕累托前沿、生产未换；
v8 从零重训四项未过线；v8r 从零+cosine lr+emb 投影块是最终裁决：
长流召回/冷启动四方最优但干净/增广全面输 v7b——从零路线不可行，
容量与初始化均非瓶颈，瓶颈在数据分布覆盖与微调先验；系列关闭，见 §4.4）。
